...
namespace AutoJTTXUtilities.AJTLoadingHandling
{
    public partial class FmFlags : Form
    {
        ...
        public void DrawStr(string str, float factor, uint ms)
        {
            宽度 = 50 * str.Length;
            ...
            //g.DrawRectangle(new Pen(Color.FromArgb(224, 224, 224)), 2, 2, 宽度 - 2 - 2, 46);//取消画矩形
            g.DrawString(str, new Font("微软雅黑", 24f / factor), new SolidBrush(Color.FromArgb(255, Color.White)), r, stringFormat);
            SetBits(bmp);
            g.Dispose();
            bmp.Dispose();
            Delay(ms);//(600u);
